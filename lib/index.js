 ...
 * @param {Object} [options] Optional settings for the import process.
 *    - `defaultOnly` (boolean): If true, only the default export of each module will be included in the result. Defaults to true.
 * @return {Object} An object containing the imported modules, with subdirectories as nested objects.
 */
export default async function importDirDynamic (dir, options = {}) {
    options.defaultOnly = options.defaultOnly === undefined ? true : options.defaultOnly
    ...
    const importPromises = itemsWithStats.map(async ({ entry, stat, path }) => {
        if (stat.isDirectory()) {
            ...
            return {
                key: name,
                value: options.defaultOnly ? module.default : module, 
                module 
            }
        }
        return null
